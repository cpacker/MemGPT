        ...
        if exception.response is not None and "application/json" in exception.response.headers.get("Content-Type", ""):
            try:
                ...
                if "error" not in error_details:
                    printd(f"HTTPError ocurred, but couldn't find error field: {error_details}")
                    return False
                else:
                ...
                if error_details.get("code") == "context_length_exceeded":
                    printd(f"HTTPError ocurred, caught error code {error_details.get('code')}")
                    return True
                # Soft-check for "maximum context length" inside of the message
                    ...
                    printd(f"HTTPError ocurred, found '{match_string}' in error message contents ({error_details})")
                    return True
                else:
                    printd(f"HTTPError ocurred, but unknown error message: {error_details}")
                    return False
            except ValueError:
