...
def upgrade() -> None:
    ...
    op.add_column("block", sa.Column("is_template", sa.Boolean(), nullable=False))
    op.add_column("block", sa.Column("organization_id", sa.String(), nullable=False))
    ...
    # Custom SQL to copy values from 'template' to 'is_template' and set to False if NULL
    op.execute(
        """
            UPDATE block
