...
class HTTPConnection(_HTTPConnection, object):
    ...
    def putrequest(self, method, url, *args, **kwargs):
        ...
        if match:
            raise ValueError("Method cannot contain non-token characters %r (found at least %r)" % (method, match.group()))
    ...
    def putheader(self, header, *values):
        ...
        if not any(isinstance(v, str) and v == SKIP_HEADER for v in values):
            ...
            raise ValueError("urllib3.util.SKIP_HEADER only supports '%s'" % ("', '".join(map(str.title, sorted(SKIPPABLE_HEADERS))),))
