...
class Factory:
    def __init__(
        ...
        if not ignore_installed:
            ...
            self._installed_dists = {dist.canonical_name: dist for dist in env.iter_installed_distributions(local_only=False)}
        else:
    ...
    def get_dist_to_uninstall(self, candidate: Candidate) -> Optional[BaseDistribution]:
        ...
        if running_under_virtualenv() and dist.in_site_packages:
            message = (
                f"Will not install to the user site because it will lack " f"sys.path precedence to {dist.raw_name} in {dist.location}"
            )
    ...
    def _report_requires_python_error(self, causes: Sequence["ConflictCause"]) -> UnsupportedPythonVersion:
        ...
        if len(causes) == 1:
            ...
            message = f"Package {causes[0].parent.name!r} requires a different " f"Python: {version} not in {specifier!r}"
