      ...
      sendMessage: ({agentId, message, role}:{agentId: string; message: string; role?: 'user' | 'system'}) => {
        ...
        void fetchEventSource(ENDPOINT_URL, {
          ...
          onmessage: async (event) => {
            ...
            const parsedData = (z.object({ internal_monologue: z.string().nullable() })
            ...
            ).and(z.object({ date: z.string().optional().transform((isoDate) => isoDate ? new Date(isoDate) : new Date()) }))
              .parse(rawData);
