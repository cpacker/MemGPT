...
class NullProvider:
    ...
    def _fn(self, base: str | None, resource_name: str):
        if base is None:
            raise TypeError("`base` parameter in `_fn` is `None`. Either override this method or check the parameter first.")
...
is not allowed.
        ...
        AttributeError: ...
        """
        invalid = os.path.pardir in path.split(posixpath.sep) or posixpath.isabs(path) or ntpath.isabs(path) or path.startswith("\\")
        if not invalid:
            return
    ...
    def check_version_conflict(self):
        ...
        for modname in self._get_metadata("top_level.txt"):
            ...
            if fn and (normalize_path(fn).startswith(loc) or fn.startswith(self.location)):
                continue
            issue_warning(
                "Module %s was already imported from %s, but %s is being added" " to sys.path" % (modname, fn, self.location),
            )
