...
def main_inner(parser, argns):
    ...
    if argns.V:
        print("Pygments version %s, (c) 2006-2023 by Georg Brandl, Matthäus " "Chajdas and contributors." % __version__)
        return 0
    ...
    if not outfn and sys.platform in ("win32", "cygwin") and fmter.name in ("Terminal", "Terminal256"):  # pragma: no cover
        ...
        try:
            ...
            outfile = colorama_initialise.wrap_stream(outfile, convert=None, strip=None, autoreset=False, wrap=True)
...
def main(args=sys.argv):
    ...
    operation.add_argument(
        ...
        action="append",
        help="Add a filter to the token stream.  (Query names with -L.) " "Filter options are given after a colon if necessary.",
    )
    operation.add_argument(
        "-f",
        metavar="FORMATTER",
    ...
    flags.add_argument(
        "--json", help="Output as JSON. This can " "be only used in conjunction with -L.", default=False, action="store_true"
    )
    ...
    special_modes.add_argument(
        ...
        help="Print style definitions for STYLE for a formatter " "given with -f. The argument given by -a is formatter " "dependent.",
    )
    special_modes.add_argument(
        "-L",
        nargs="*",
    ...
    special_modes.add_argument(
        "-C", action="store_true", help="Like -N, but print out a lexer name based solely on " "a given content from standard input."
    )
    special_modes.add_argument(
        "-H",
        action="store",
        nargs=2,
        metavar=("NAME", "TYPE"),
        help="Print detailed help for the object <name> of type <type>, " 'where <type> is one of "lexer", "formatter" or "filter".',
    )
