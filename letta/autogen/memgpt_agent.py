...
from letta.agent import Agent as Agent
from letta.agent import save_agent
from letta.agent_store.storage import StorageConnector, TableType
from letta.autogen.interface import AutoGenInterface
from letta.cli.cli_load import load_directory, load_vector_database
from letta.config import LettaConfig
from letta.credentials import LettaCredentials
from letta.data_types import EmbeddingConfig, LLMConfig, User
from letta.metadata import MetadataStore
from letta.utils import get_human_text, get_persona_text


class LettaConversableAgent(ConversableAgent):
    def __init__(
        self,
        name: str,
        agent: Agent,
        ...
        """A wrapper around a Letta agent that implements the AutoGen ConversibleAgent functions

        This allows the Letta agent to be used in an AutoGen groupchat
        """
        ...
        self.register_reply([Agent, None], LettaConversableAgent._generate_reply_for_user_message)
    ...
    def save(self):
        """Save the underlying Letta agent to the database"""
        try:
            save_agent(agent=self.agent, ms=self.ms)
        except Exception as e:
            print(f"Failed to save Letta AutoGen agent\n{self.agent}\nError: {str(e)}")
            raise
    ...
    def _generate_reply_for_user_message(
        ...
        ), f"Letta AutoGen Agent is using the wrong interface - {self.agent.interface}"
        ...
        if new_messages_count > 1:
            if self.concat_other_agent_messages:
                ...
                # Extend the Letta message list with multiple 'user' messages, then push the last one with agent.step()
        ...
        # Send a single message into Letta
        while True:
            (
                new_messages,
                heartbeat_request,
        ...
        # Pass back to AutoGen the pretty-printed calls Letta made to the interface
        pretty_ret = LettaConversableAgent.pretty_concat(self.agent.interface.message_list)
    ...
    def pretty_concat(messages):
        """AutoGen expects a single response, but Letta may take many steps.

        To accommodate AutoGen, concatenate all of Letta's steps into one and return as a single message.
        """
        ...
        # prevent error in LM Studio caused by scenarios where Letta didn't say anything
        if ret["content"] in ["", "\n"]:
            ret["content"] = "..."
...
def load_autogen_letta_agent(
    ...
    default_auto_reply: Optional[Union[str, Dict, None]] = "",
    is_termination_msg: Optional[Callable[[Dict], bool]] = None,
) -> LettaConversableAgent:
    """Load a Letta agent into a wrapped ConversableAgent class"""
    ...
    try:
        letta_agent = Agent(agent_state=agent_state, interface=interface)
    except Exception:
    ...
    llm_config_was_modified = update_config_from_dict(letta_agent.agent_state.llm_config, agent_config)
    embedding_config_was_modified = update_config_from_dict(letta_agent.agent_state.embedding_config, agent_config)
    ...
    autogen_letta_agent = LettaConversableAgent(
    ...
    return autogen_letta_agent


def create_autogen_letta_agent(
    agent_config: dict,
    skip_verify: bool = False,
    auto_save: bool = False,
    interface: bool = None,
    ...
    `interface_kwargs`.  For example, Letta's inner monologue and functions are
    off by default so that they are not visible to the other agents. You can
    turn these on by passing in
    ```
    interface_kwargs={
        "debug": True,  # to see all Letta activity
        "show_inner_thoughts: True  # to print Letta inner thoughts "globally"
    ...
    try:
        ...
        if preset_obj is None:
            ...
            from letta.presets.presets import add_default_presets

            add_default_presets(user.id, ms)
            # try again
            preset_obj = ms.get_preset(name=agent_config["preset"] if "preset" in agent_config else config.preset, user_id=user.id)
            if preset_obj is None:
                print("Couldn't find presets in database, please run `letta configure`")
                sys.exit(1)
    ...
    # After creating the agent, we then need to wrap it in a ConversableAgent so that it can be plugged into AutoGen
    autogen_letta_agent = LettaConversableAgent(
        name=letta_agent.agent_state.name,
        agent=letta_agent,
        default_auto_reply=default_auto_reply,
        is_termination_msg=is_termination_msg,
        skip_verify=skip_verify,
...
def create_letta_autogen_agent_from_config(
    name: str,
    ...
    # config setup for non-letta agents:
    nonletta_llm_config: Optional[Union[Dict, bool]] = None,
    default_auto_reply: Optional[Union[str, Dict, None]] = "",
    interface_kwargs: Dict = None,
    skip_verify: bool = False,
    auto_save: bool = False,
) -> LettaConversableAgent:
    """Same function signature as used in base AutoGen, but creates a Letta agent
    ...
    # The "system message" in AutoGen becomes the persona in Letta
    ...
    autogen_letta_agent = create_autogen_letta_agent(
        agent_config,
        default_auto_reply=default_auto_reply,
        is_termination_msg=is_termination_msg,
    ...
    if human_input_mode != "ALWAYS":
        coop_agent1 = create_autogen_letta_agent(
            agent_config,
            default_auto_reply=default_auto_reply,
            is_termination_msg=is_termination_msg,
            interface_kwargs=interface_kwargs,
        ...
        if default_auto_reply != "":
            ...
            coop_agent2 = create_autogen_letta_agent(
                agent_config,
                default_auto_reply=default_auto_reply,
                is_termination_msg=is_termination_msg,
                interface_kwargs=interface_kwargs,
        ...
        groupchat = GroupChat(
            agents=[autogen_letta_agent, coop_agent1, coop_agent2],
            messages=[],
            max_round=12 if max_consecutive_auto_reply is None else max_consecutive_auto_reply,
        )
        assert nonletta_llm_config is not None
        manager = GroupChatManager(name=name, groupchat=groupchat, llm_config=nonletta_llm_config)
        return manager
