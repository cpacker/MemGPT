    ...
    def _clean_agent_state(self, agent_state: DataAgentState, action: str = "create") -> DataAgentState:
        ...
        if agent_state.tools:
            splatted_pydantic["tools"] = [
                SQLTool.read(self.db_session, name=r) if not isinstance(r, Tool) else r.to_sqlalchemy(self.db_session)
                for r in agent_state.tools
            ]
        if agent_state.message_ids and action == "create":
            splatted_pydantic["messages"] = [
                SQLMessage.read(self.db_session, identifier=r) if not isinstance(r, Message) else r.to_sqlalchemy(self.db_session)
