...
def upgrade() -> None:
    ...
    op.add_column("organization", sa.Column("deleted", sa.Boolean(), nullable=True))

    # loop through all rows in the organization table and set the deleted column to False
    op.execute("UPDATE organization SET deleted = FALSE")

    # set the deleted column to not nullable
    op.alter_column("organization", "deleted", existing_type=sa.Boolean(), nullable=False)
    ...
    op.add_column("tool", sa.Column("deleted", sa.Boolean(), nullable=True))

    # loop through all rows in the tool table and set the deleted column to False
    op.execute("UPDATE tool SET deleted = FALSE")

    # set the deleted column to not nullable
    op.alter_column("tool", "deleted", existing_type=sa.Boolean(), nullable=False)
    ...
    # Remove all tools, because we need to pair them with organizations
    ...
    # loop through all rows in the user table and set the deleted column to False
    op.execute('UPDATE "user" SET deleted = FALSE')
    ...
    op.add_column("user", sa.Column("is_deleted", sa.Boolean(), server_default=sa.text("FALSE"), nullable=True))

    # loop through all rows in the user table and set the is_deleted column to False
    op.execute('UPDATE "user" SET is_deleted = FALSE')

    # set the is_deleted column to not nullable
    ...
    op.add_column("user", sa.Column("_organization_id", sa.String(), nullable=True))

    # loop through all rows in the user table and set the _organization_id column from organization_id
    op.execute('UPDATE "user" SET _organization_id = org_id')

    # set the _organization_id column to not nullable
    op.alter_column("user", "_organization_id", existing_type=sa.String(), nullable=False)
