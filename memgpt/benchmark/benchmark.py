...
def bench(
    ...
    print(f"\nDepending on your hardware, this may take up to 30 minutes. This will also create {TRIES * len(PROMPTS)} new agents.\n")
    ...
    for fn_type, message in PROMPTS.items():
        ...
        for i in range(TRIES):
            ...
            agent_id = agent.id
            result, msg, tokens_accumulated = send_message(message=message, agent_id=agent_id, turn=i, fn_type=fn_type, print_msg=print_messages)
        ...
        print(f"Score for {fn_type}: {score}/{TRIES}, took {elapsed_time_run} seconds")

        elapsed_time += elapsed_time_run
        total_score += score     

    config = MemGPTConfig(agent=agent_id)
    ...
    print(f"\n\t-> Total score: {total_score}/{len(PROMPTS) * TRIES}, took {elapsed_time} seconds at average of {round(total_tokens_accumulated/elapsed_time, 2)} t/s\n")
