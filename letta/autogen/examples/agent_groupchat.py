"""Example of how to add Letta into an AutoGen groupchat
...
# If USE_MEMGPT is True, then we swap out the "coder" agent with a Letta agent
USE_MEMGPT = True
...
if not USE_MEMGPT:
    ...
    coder = create_letta_autogen_agent_from_config(
        "Letta_coder",
        ...
        auto_save=False,  # Set this to True if you want the Letta AutoGen agent to save its internal state after each reply - you can also save manually with .save()
    )
    ...
    # coder = load_autogen_letta_agent(agent_config={"name": "Letta_coder"})
