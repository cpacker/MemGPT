...
class AutoGenInterface(object):
    """AutoGen expects a single action return in its step loop, but Letta may take many actions.
    ...
    The buffer needs to be wiped before each call to letta.agent.step()
    """
    ...
    def reset_message_list(self):
        """Clears the buffer. Call before every agent.step() when using Letta+AutoGen"""
        self.message_list = []
    ...
    def internal_monologue(self, msg: str, msg_obj: Optional[Message] = None):
        ...
        message = f"\x1B[3m{Fore.LIGHTBLACK_EX}💭 {msg}{Style.RESET_ALL}" if self.fancy else f"[Letta agent's inner thoughts] {msg}"
        print(message)
