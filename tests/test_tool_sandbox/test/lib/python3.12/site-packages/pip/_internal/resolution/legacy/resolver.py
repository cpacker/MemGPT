...
def _check_dist_requires_python(
    ...
    raise UnsupportedPythonVersion(f"Package {dist.raw_name!r} requires a different Python: " f"{version} not in {requires_python!r}")
...
class Resolver(BaseResolver):
    ...
    def _add_requirement_to_set(
        ...
        if has_conflicting_requirement:
            raise InstallationError(f"Double requirement given: {install_req} " f"(already in {existing_req}, name={install_req.name!r})")
    ...
    def _get_dist_for(self, req: InstallRequirement) -> BaseDistribution:
        ...
        if req.satisfied_by:
            should_modify = (
                self.upgrade_strategy != "to-satisfy-only" or self.force_reinstall or self.ignore_installed or req.link.scheme == "file"
            )
            if should_modify:
