...
async def assistant_message(msg):
    ...
    # The URL you want to post to (this should be the URL where your Flask server is running)
    ...
    # Adding headers to specify that content type is JSON
    headers = {'Content-Type': 'application/json'}

    # Making the POST request
    response = requests.post(url, headers=headers, data=json.dumps(data_to_post))

    # Assuming that the Flask server returns JSON data, you can print the response like so:
    if response.status_code == 200:  # HTTP status code 200 means OK, request succeeded
        json_response = response.json()
        print("Server responded with:", json_response)
    else:
