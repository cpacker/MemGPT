...
def test_add_tag_to_agent(client: Union[LocalClient, RESTClient], agent: AgentState):
    # Add a new tag to the agent
    tag = "test_tag"
    added_tag = client.add_tag_to_agent(agent_id=agent.id, tag=tag)

    # Verify the tag was added correctly
    assert added_tag.agent_id == agent.id
    assert added_tag.tag == tag

    # Retrieve tags for the agent to confirm addition
    tags = client.get_tags_for_agent(agent_id=agent.id)
    assert tag in tags, f"Tag {tag} not found in tags {tags}"


def test_delete_all_tags_from_agent(client: Union[LocalClient, RESTClient], agent: AgentState):
    # Setup: Add multiple tags to the agent
    client.add_tag_to_agent(agent_id=agent.id, tag="tag1")
    client.add_tag_to_agent(agent_id=agent.id, tag="tag2")

    # Ensure the tags were added
    tags = client.get_tags_for_agent(agent_id=agent.id)
    assert "tag1" in tags
    assert "tag2" in tags
    # Delete all tags from the agent
    client.delete_all_tags_from_agent(agent_id=agent.id)

    # Verify all tags are deleted
    tags = client.get_tags_for_agent(agent_id=agent.id)
    assert len(tags) == 0, f"Tags not deleted, found {tags}"


def test_delete_tag_from_agent(client: Union[LocalClient, RESTClient], agent: AgentState):
    # Add a specific tag to the agent
    tag = "specific_tag"
    client.add_tag_to_agent(agent_id=agent.id, tag=tag)

    # Ensure the tag was added
    tags = client.get_tags_for_agent(agent_id=agent.id)
    assert tag in tags, f"Tag {tag} not found in {tags}"

    # Delete the specific tag from the agent
    client.delete_tag_from_agent(agent_id=agent.id, tag=tag)

    # Verify the tag is removed
    tags = client.get_tags_for_agent(agent_id=agent.id)
    assert tag not in tags, f"Tag {tag} still present after deletion in {tags}"


def test_get_agents_by_tag(client: Union[LocalClient, RESTClient], agent: AgentState):
    # Setup: Add a unique tag to the agent
    unique_tag = "unique_test_tag"
    client.add_tag_to_agent(agent_id=agent.id, tag=unique_tag)

    # Retrieve agents by the unique tag
    agents = client.get_agents_by_tag(tag=unique_tag)

    # Verify the agent ID is in the list of agents for the tag
    assert agent.id in agents, f"Agent {agent.id} not found in agents {agents}"


def test_get_tags_for_agent(client: Union[LocalClient, RESTClient], agent: AgentState):
    # Setup: Add multiple tags to the agent
    tags_to_add = ["tag1", "tag2", "tag3"]
    for tag in tags_to_add:
        client.add_tag_to_agent(agent_id=agent.id, tag=tag)

    # Retrieve tags for the agent
    tags = client.get_tags_for_agent(agent_id=agent.id)

    # Verify all tags were retrieved correctly
    assert set(tags_to_add).issubset(set(tags)), f"Expected tags {tags_to_add} in {tags}"
